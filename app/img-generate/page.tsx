...
export default function ImageGeneratePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const { data } = await axios.post("/api/img-generate", { prompt: userMessage });
      if (data.error) {
        setError(data.error);
      } else {
        setMessages((prev) => [
          ...prev,
          { role: "assistant", content: data, type: "image" },
        ]);
      }
    } catch (err) {
  ...
  return (
    <div className="min-h-screen p-8 max-w-2xl mx-auto">
      <h1 className="text-2xl font-bold mb-6">Image Generate</h1>
